...
export default function AuditLog() {
  ...
  const filtered = logs.filter(l => {
    ...
    if (search && !l.asset_name?.toLowerCase().includes(search.toLowerCase()) &&
        !l.performed_by?.toLowerCase().includes(search.toLowerCase())) return false
    return true
  })
    ...
    <>
      ...
      <div className="stat-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)', marginBottom: '1.5rem' }}>
        <div className="stat-card">
          <div className="stat-num green">{counts.created}</div>
          <div className="stat-label">Assets added</div>
        </div>
        <div className="stat-card">
          <div className="stat-num amber">{counts.updated}</div>
          <div className="stat-label">Assets updated</div>
        </div>
        <div className="stat-card">
          <div className="stat-num red">{counts.deleted}</div>
          <div className="stat-label">Assets deleted</div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--muted)', fontSize: 13 }}>
            No audit events yet. Asset additions, updates and deletions will appear here.
          </div>
          ...
          <div key={entry.id} style={{
            ...
            <div style={{
              ...
              background: auditBg[entry.action],
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 13, color: auditColor[entry.action], flexShrink: 0, marginTop: 2,
            }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 500,
                  color: entry.asset_id ? 'var(--text)' : 'var(--muted)',
                  cursor: entry.asset_id ? 'pointer' : 'default',
                  textDecoration: entry.asset_id ? 'underline' : 'none',
                  textDecorationColor: 'var(--border)',
                }}
                  onClick={() => entry.asset_id && navigate(`/assets/${entry.asset_id}`)}>
                  {entry.asset_name}
                </span>
                <span style={{
                  fontSize: 11, padding: '2px 8px', borderRadius: 4, fontWeight: 500,
                  background: auditBg[entry.action], color: auditColor[entry.action],
              ...
              {entry.reason && (
                <div style={{
                  fontSize: 12, color: 'var(--muted)', padding: '8px 10px',
                  background: 'var(--cream)', borderRadius: 6, lineHeight: 1.5,
                }}>
                  {entry.reason}
                </div>
              )}
              {entry.changes && Object.keys(entry.changes).length > 0 && (
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6 }}>
                  {Object.entries(entry.changes).map(([k, v]) => (
                    <span key={k} style={{
                      fontSize: 11, padding: '2px 8px', background: 'var(--cream2)',
